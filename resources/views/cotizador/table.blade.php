...
<style type="text/css">
	...
	<table class="table table-striped small row-border" style="font-size: 13px;" id="" border="0">
		<tr style="border-top: 2px solid white; background:white;">
			<td colspan="{{ $productos[0]->estatus == 1 ? 11:9}}"><span class="badge badge-primary">Cotización {{ $num_cotizacion}}</span></td>
		...
		<tr>
			...
			<td ><span class="badge badge-primary">{{ $p->inv1}}</span> <input type="hidden" id="cin2{{$p->id}}" value="{{ $p->inv1}}"></td>
			<td> <span class="badge badge-primary">{{ $p->inv2}}</span> <input type="hidden" id="cin1{{$p->id}}" value="{{ $p->inv2}}"> </td>
			<td>
			...
			@if($p->estatus== 1)
			<td>
				<input type="text" id="cantinv1{{$p->id}}" value="{{$p->inventa1}}" class="form-control form-control-sm cantidad-mask text-right"  onchange="guarda_info_cotizacion({{$p->id}})" style="width: 50px;">
			</td>
			<td><input type="text" id="cantinv2{{$p->id}}" value="{{$p->inventa2}}" class="form-control form-control-sm cantidad-mask text-right"  onchange="guarda_info_cotizacion({{$p->id}})" style="width: 50px;"></td>
			@endif
		...
		@endforeach
		<tr>
			<td colspan="{{ $productos[0]->estatus == 1 ? 12:10}}" class="color" rowspan="6"></td>
